    ...
    if savemodel:
        if model_init:
            if not os.path.exists("./models"):
                os.makedirs("./models")
            model.save_model("./models/model_init.json")
        else:
            model.save_model("./models/model_{}.json".format(checkpoint))
    return model
...
def train_model(model, params, useDataset=False, steps=100):
    ...
    if not useDataset:
        ...
        if not os.path.exists("../../static/dataset/mvptest"):
            ...
            model = xgb.train(params, D_train, steps, evals=[(D_train, "train"), (D_val, "validation")],
                             early_stopping_rounds=50)
        else:
            #TODO: read local dataset directly
            pass
        return model
...
if __name__ == "__main__":
    ...
    train(useDataset=True, savemodel=False)
